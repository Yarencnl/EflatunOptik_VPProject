...
{
    public partial class MainPageFormK : Form
    {
        public MainPageFormK()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            using (var baglanti = new SQLiteConnection(VeriTabaniIslemleri.BaglantiCumlesi))
            {
                ...
                using (var komut = new SQLiteCommand(sql, baglanti))
                {
                    ...
                    if (etkilenenSatir > 0)
                        MessageBox.Show($"{kategori} Gözlük ({secilenBeden} Beden) satın alındı. Stok güncellendi!");
                    else
